...
public class DvbSubtitlesParser {
  public static final int BITMAP_NOT_PAINTED = 0;
  public static final int BITMAP_PAINTED = 1;
  ...
  DvbSubsRendererState rendererState;
  int adaptationField;
  int numTSPackets;
  //endOfPacketReachedCallback getMoreData;
  private Paint graphicsPaintObject;
  private Canvas graphicsCanvasObject;
  private Bitmap graphicsBitmapObject;
  static ParsableBitArray tsStream;
  private DvbSubColor color;
      ...
      if (cluts != null) {
        for (int i = 0; i < cluts.length; i++) {
          if (cluts[i].id == received.id) {
            cluts[i] = received;
            System.out.println("overwriting already stored region");
            return;
          }
        }
      }
  ...
  private void parseDDSegment(DvbSubDisplay display) {
    ...
    rendererState.displayWidth = display.width;
    display.height = tsStream.readBits(16);
    rendererState.displayHeight = display.height;
    if (display.windowed == 1) {
      display.x = tsStream.readBits(16);
      rendererState.displayHorizontal = display.x;
      display.maxX = tsStream.readBits(16);
      display.y = tsStream.readBits(16);
      rendererState.displayVertical = display.y;
  ...
  private int parseObjectSegment(DvbSubObjectDef object) {
    ...
    previousCoordX = x; previousCoordY = y;
    tsStream.skipBits(1);
...
//            }
  ...
  public DvbSubtitlesParser   (Paint paint, Bitmap bitmap) {
    ...
    graphicsPaintObject = paint;
    graphicsBitmapObject = bitmap;
    graphicsCanvasObject = new Canvas(graphicsBitmapObject);
  ...
  public DvbSubsRendererState parse(SampleHolder sampleHolder, Bitmap bitmap, Canvas canvas) {
    graphicsBitmapObject = bitmap;
    graphicsCanvasObject = canvas;
    if (sampleHolder.data != null) {
      tsStream = new ParsableBitArray(sampleHolder.data.array());
    } else {
      rendererState.bitmapPainted = BITMAP_NOT_PAINTED;
    ...
    if (sync == 0xff) {
      ...
      if (newGraphicData) {
        ...
        newGraphicData = false;
        bitmap = graphicsBitmapObject;
        rendererState.bitmapPainted = BITMAP_PAINTED;
        return rendererState;
      } else {
